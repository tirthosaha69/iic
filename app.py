...
# ─────────────────────────────────────────────
GROQ_API_KEY = "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"
OWM_API_KEY  = "07ae890582be15d21bbe4f8fa989559c"   # <-- replace with your key from openweathermap.org
...
try:
    ...
    _llm = None
    print(f"[STARTUP ERROR] LLM failed: {e}")
...
def parse_llm_json(raw: str) -> dict:
    ...
    if start == -1 or end == -1:
        raise ValueError(f"No JSON found in LLM response: {cleaned[:300]}")
...
CITY_COORDS = {
    ...
    "north 24 parganas":(22.7000, 88.4000),
    "south 24 parganas":(22.1000, 88.5000),
...
def get_coords(location):
    """Return (lat, lon) for a location string. Python 3.8+ compatible."""
    ...
    try:
        r = requests.get(
            ...
            params={"q": location + ",IN", "limit": 1, "appid": OWM_API_KEY},
            timeout=5
        )
        data = r.json()
...
def fetch_current_weather(lat, lon):
    """Fetch current weather from OWM. Returns empty dict on failure."""
    try:
        r = requests.get(
...
def fetch_forecast(lat, lon):
    """Fetch 5-day / 3-hour forecast from OWM. Returns empty dict on failure."""
    try:
        r = requests.get(
        ...
        avg_temps = {day_key: round(sum(t) / len(t), 1) for day_key, t in daily_temp.items()}

        return {
            "total_5day_rain_mm": round(total_5day_rain, 1),
            "daily_rain":         {day_key: round(daily_rain[day_key], 1) for day_key in days},
...
def fetch_air_quality(lat, lon):
    """Fetch AQI from OWM. Returns empty dict on failure."""
    try:
        r = requests.get(
...
def build_weather_context(location):
    """Fetch all OWM data for a location and return a combined dict."""
...
{{
  ...
  "alerts": [
    "<specific alert using real temp {wx['current'].get('temp','?')}C and rain {wx['current'].get('rain_3h',0)}mm>",
...
High temp + low rain = high agricultural stress. Low rainfall over 5 days = increasing stress index.
Generate real district names from {location}. Be specific.
...
{{
  ...
  "surface_water_availability": "<Normal|Below Average|Well Below Average|Above Average>",
  ...
  "stress_forecast_30d": [<8 integers showing trend>],
...
High AQI, NO2, SO2 = industrial pollution. High NH3 = agricultural/sewage runoff.
...
def get_drought_forecast():
    ...
    daily_rain_json = json.dumps(wx['forecast'].get('daily_rain', {}))
    daily_temp_json = json.dumps(wx['forecast'].get('daily_avg_temp', {}))
...
- Daily Rain Forecast:{daily_rain_json}
- Daily Avg Temp:     {daily_temp_json}
...
{{
  ...
  "forecast_90d": {{
    "outlook": "<specific forecast for {location}>",
  ...
  "affected_districts": [<3-5 real districts in {location} most affected>],
...
{{
  ...
  "state": "<state>",
  "summary": "<2 specific sentences about current water/weather situation based on the live data above>",
  "location_info": {{
    "temp":        "{wx['current'].get('temp', '?')}C",
    "rainfall":    "{wx['current'].get('rain_3h', 0)} mm",
  ...
  "reservoir":      {{"capacity_pct": <estimate>, "trend": "<trend>", "status": "<status>"}},
...
# ─────────────────────────────────────────────
# RUN — must be at the very end of the file
# ─────────────────────────────────────────────
